...
class ResNet1D_Seq(nn.Module):
    def __init__(self,
                 in_channels=6,          # T,Q,V values + T,Q,V masks
                 base_filters=64,
                 kernel_size=7,
                 groups=1,
                 n_block=12,
        ...
        self.n_block = n_block
        self.kernel_size = kernel_size
        self.stride = 1           # keep stride=1 so output length == input length
        self.groups = groups
        self.use_bn = use_bn
        self.use_do = use_do
        ...
        for i_block in range(self.n_block):
            ...
            # BUT since stride=1, downsample path will be a no-op (pool k=1)
            if is_first_block:
                in_ch = base_filters
                out_ch = in_ch
            else:
                in_ch = int(base_filters * 2 ** ((i_block - 1) // self.increasefilter_gap))
                out_ch = in_ch * 2 if ((i_block % self.increasefilter_gap == 0) and (i_block != 0)) else in_ch

            self.basicblock_list.append(BasicBlock(
                in_channels=in_ch, out_channels=out_ch,
                kernel_size=self.kernel_size, stride=self.stride,
                groups=self.groups, downsample=downsample,
                use_bn=self.use_bn, use_do=self.use_do, is_first_block=is_first_block
            ))
        self.final_bn   = nn.BatchNorm1d(out_ch)
        self.final_relu = nn.ReLU(inplace=True)
        # seq head: predict 3 modalities at each position
        self.head = nn.Conv1d(out_ch, 3, kernel_size=1)

    def forward(self, x):   # x: [B, C_in, L]
        out = x
        if self.verbose: print('input', out.shape)
        out = self.first_block_conv(out)
        if self.verbose: print('after first conv', out.shape)
        if self.use_bn: out = self.first_block_bn(out)
        out = self.first_block_relu(out)

        for i, block in enumerate(self.basicblock_list):
            if self.verbose:
                print(f'block {i}: in {block.in_channels}, out {block.out_channels}, down={block.downsample}')
            out = block(out)

        if self.use_bn: out = self.final_bn(out)
        out = self.final_relu(out)        # [B, C, L]
        out = self.head(out)              # [B, 3, L]   -> (T,Q,V)
        return out