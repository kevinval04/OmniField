...
class ConvolutionalBlock(nn.Module):
    def __init__(
        ...
        if padding is None:  
            ...
            # Fall back to manual padding if explicitly passed
            total_padding = kernel_size + 2 * (dilation - 1) - 1
            padding_val = total_padding // 2 if padding else 0
            conv_layer = conv_class(
                in_channels,
                out_channels,
                kernel_size,
...
class DecodingBlock(nn.Module):
    def __init__(
        ...
        if upsampling_type == "conv":
            ...
            # we’ll use interpolate with target size instead of scale_factor
            self.upsample = None  
    ...
    def forward(self, skip_connection, x):
        # if using ConvTranspose, make sure to resize result explicitly to match skip
        if self.upsample is None:
            target_size = skip_connection.shape[2:]
            x = F.interpolate(x, size=target_size, mode=self.upsampling_type, align_corners=False)
        else:
            x = self.upsample(x)
            # explicitly match skip size after conv transpose
            target_size = skip_connection.shape[2:]
            if x.shape[2:] != target_size:
                x = F.interpolate(x, size=target_size, mode="linear" if x.dim() == 3 else "nearest")
...
class UNet1DTQV(nn.Module):
    def __init__(self, latent_dim=128, base_channels=64):
        super().__init__()
        # your encoders / decoder setup...
        self.enc_T = UNet1D(in_channels=1, out_classes=latent_dim)
        self.enc_Q = UNet1D(in_channels=1, out_classes=latent_dim)
        self.enc_V = UNet1D(in_channels=1, out_classes=latent_dim)
        self.decoder = UNet1D(in_channels=latent_dim, out_classes=3)

    def forward(self, T, Q, V):
        zT = self.enc_T(T)
        zQ = self.enc_Q(Q)
        zV = self.enc_V(V)
        z = (zT + zQ + zV) / 3.0
        out = self.decoder(z)  # [B, C, L_pred]

        # --- force match GT length ---
        target_len = T.shape[-1]  # 21600
